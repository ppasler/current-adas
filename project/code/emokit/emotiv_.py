...
tasks = Queue()
dataQueue = Queue()
epoc_data = {"Y": {"quality": 0, "value": 23}, "F3": {"quality": 0, "value": -32},
"F4": {"quality": 0, "value": 1349}, "P7": {"quality": 0, "value": -91},
"FC6": {"quality": 0, "value": 107}, "F7": {"quality": 0, "value": -68},
"F8": {"quality": 0, "value": -30}, "T7": {"quality": 0, "value": -45},
"P8": {"quality": 0, "value": -2}, "FC5": {"quality": 0, "value": -11},
"AF4": {"quality": 0, "value": 23}, "Unknown": {"quality": 0, "value":
58}, "T8": {"quality": 0, "value": 77}, "X": {"quality": 0, "value":
22}, "O2": {"quality": 0, "value": -309}, "O1": {"quality": 0, "value":
152}, "AF3": {"quality": 0, "value": -107}}
...
class Emotiv(object):
    ...
    """
    def __init__(self, serial_number="", is_research=False):
        """
    ...
    def setup_windows(self):
        ...
        try:
            ...
            server_updater = gevent.spawn(self.update_server)
            ...
            gevent.kill(server_updater, KeyboardInterrupt)
            self.server.stop()
    ...
    def update_console(self):
        ...
        while self.running:
            ...
            print('\n'.join("%s Reading: %s Quality: %s" % (k[1], self.sensors[k[1]]['value'], self.sensors[k[1]]['quality']) for k in enumerate(self.sensors)))
        ...
        with open(fname, 'a') as f:
            while self.running:
                while not dataQueue.empty():
                    sensor = dataQueue.get()
                    f.write(str(int(round(time.clock() * 1000)) + start))
                    for k in enumerate(sensor):
                        f.write(';' + str(sensor[k[1]]['value']))
                    for k in enumerate(sensor):
                        f.write(';' + str(sensor[k[1]]['quality']))
                    f.write('\n')
...
class EpocServer(object):
    def __init__(self, host="localhost", port=9000):
        ...
        self.server_class = SocketServer.TCPServer
