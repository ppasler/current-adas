...
\sloppypar{
\begin{abstract}
Mit Fortschreiten der Technik verbreiten sich \acl{FAS}en immer weiter - auch die \acl{ME} ist ein solches System. In einem kurzen Überblick, werden verschiedene Funktionsweisen und Ansätze von Systemen zur \acl{ME} vorgestellt. Die  Erkennung mit Body-Sensorik wird auf seine Umsetzbarkeit im Simulationsumfeld der \acl{RTU} evaluiert. Weiterhin wird die Möglichkeit einer einfachen Portieren der Anwendung vom Simulator in ein echtes Fahrzeug geprüft.
...
\section{Einleitung}
\label{chap:intro}
Fristeten \acl{FASs} vor wenigen Jahren ein Nischendasein in Oberklassewagen, werden sie immer günstiger und beliebter. So halten sie auch in Mittel- und Kleinwagen Einzug und helfen bei der Vermeidung schwerer Unfälle. 

Um ein neues System auszuprobieren, setzen Automobilhersteller, wie auch Wissenschaftler, auf Tests in einem Simulator (unter Laborbedingungen), bevor es in echte Fahrzeuge integriert werden. 

In der vorgestellten Arbeit wird die \acl{ME} als Vertreter eines \acl{FAS}s näher beleuchtet und verschieden Umsetzungsvorschläge gegenübergestellt (Kapitel \ref{chap:me}). In Kapitel \ref{chap:prop} wird ein solches System mit Körpersensoren (EEG, EKG) vorgestellt und in Kapitel \ref{chap:eval} die Umsetzbarkeit im Simulationsumfeld der \acl{RTU} evaluiert. Das Ergebnis und  weitere Schritte werden in Kapitel \ref{chap:result} und \ref{chap:outro} beschrieben.\\

\textbf{\acl{FASs}}\\
- Studie zu Unfällen -
 \cite{Feld:2010:MIA:1719970.1720063}

\acl{FASs} bieten dem Fahrer zum einen ein Komfortplus oder erhöhen die Sicherheit beim Fahren. So führen Einparkassistent,  Geschwindigkeitsregelanlage oder Navigation zu einer deutlich entspannteren Fahrt. Spurhalte-, Spurwechsel- oder Notbremsassistent wiederum unterstützen bei potentiell gefährlichen Manövern. Auch die \acl{ME} fällt in die zweite Kategorie (mehr dazu in Kapitel \ref{chap:me}).\\
